...
"""Run DJINN multiple-output regression examples.

This script demonstrates training and evaluating both standard and Bayesian
DJINN regressors for a two-output target derived from the Boston dataset.
"""
...
def main():
    ...
    axs.scatter(y_test[:, 0], bm[:, 0], linewidth=0, s=6, alpha=0.8, c="#68d1ca")
    a, b, c = axs.errorbar(
        y_test[:, 0],
        bm[:, 0],
        yerr=[bm[:, 0] - bl[:, 0], bu[:, 0] - bm[:, 0]],
        marker="",
        ls="",
        zorder=0,
        alpha=0.5,
    ...
    fig, axs = plt.subplots(1, 1, figsize=(8, 8), facecolor="w", edgecolor="k")
    fig.subplots_adjust(hspace=0.15, wspace=0.1)
    axs.scatter(y_test[:, 1], bm[:, 1], linewidth=0, s=6, alpha=0.8, c="#68d1ca")
    a, b, c = axs.errorbar(
        y_test[:, 1],
        bm[:, 1],
        yerr=[bm[:, 1] - bl[:, 1], bu[:, 1] - bm[:, 1]],
        marker="",
        ls="",
        zorder=0,
        alpha=0.5,
